...
class NormalActionsTest(BaseAction):
    ...
    def test_do_deactivate_realm(self) -> None:
        ...
        # state_change_expected is False as deactivating realm does not require
        # state changes. We delete sessions of all active users when a realm is
        # deactivated, and redirect them to a deactivated page in order to inform
        # that realm/organization has been deactivated, it's because that user
        # can't run the app that we don't need to change states and send `page_params`.
